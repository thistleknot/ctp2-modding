...
```
Full regen sequence (real-art install + proxy art are required for merged mods;
run the generator TWICE after a full rebuild — pass 1 prices advances from
pre-sever prereq counts, pass 2 is the byte-stable fixed point):
```
rmdir /s /q scen0000 & xcopy /e /i ..\mom\scen0000 scen0000   # base scenario
python ..\mom\tools\assign_proxy_art.py --scenario scen0000 --csv csv --art-dir art\pictures
...
```
Real unit art lives in `art\pictures\` (durable, git-tracked), built from the
CoMM3 HoMM3 archive by:
```
python ..\mom\tools\build_unit_icon_art.py --csv csv --archive H:\Games\ctp2\CoMM3.7z --out art\pictures
```
`csv\unit_art_map.csv` is the reviewable staging sheet (unit -> archive art);
edit `archive_path`/`kind` per row and re-run with --force to re-render. v1:
85/477 merged units mapped (HoMM fantasy core); unmapped units keep proxies.
