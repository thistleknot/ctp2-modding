...
**Implementation status (verified 2026-07-16):** the union primitive (`merge_control_planes.py`) IS deterministic first-wins on sanitized name + `tag:code` namespacing — no LLM in the mechanical pipeline; the REDUCE lives in curated staging sheets edited between merge passes. Audit surfaces now exist: `collision_ledger.csv` (every first-wins-dropped row, written by the merge) and `collapse_candidates.csv` (`audit_collapse_candidates.py`, data-derived faction-suffix stemming → cross-mod same-concept groups awaiting the step-4 REDUCE decision). 7-source SMM merge verified deterministic + referentially clean; see lessons_learned 2026-07-16 entry.
